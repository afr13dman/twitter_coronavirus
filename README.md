...
However, in 2022, the structure for saving each tweet changed so from January 1st to April 5th tweets were saved as `geoTwitterYY-MM-DD.zip` and afterwards they were saved as `tweets-YYYYMMDD.zip`.
...
Since there are 365 days, or files, worth of data, I created a shell script to loop over each in the dataset and run the `map.py` command on that file.
...
I used output redirect to write the outputs for each day to a unique file. This allowed me to check if an error occured when running `map.py`.
Rather than open each output file and read it, I created a shell script `check_nohup.sh` to run through each file and output a new file with which dates had errors when `map.py` was run.

Once I fixed all errors, I merged the outputs generated by the `map.py` file to combine all of the `.lang` files into a single file, and all of the `.country` files into a different file.
```
